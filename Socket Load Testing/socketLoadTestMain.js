...
const createClient = () => {
    ...
    const transports = Math.random() < POLLING_PERCENTAGE ["websocket","polling"];
    //console.log(student[0])
    //logger.info("User Created  - username" + student[clientCount]['username'])
    const socket = io(URL + "?token=" + student[clientCount]["access_token"] + "&user_id=" + student[clientCount]["userID"], {
        transports, reconnectionDelay: 1000, forceNew: true, timeout: 8000
    });
